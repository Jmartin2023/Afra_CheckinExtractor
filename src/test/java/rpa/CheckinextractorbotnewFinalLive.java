...
/**
 * Daily bot (30-day window) with PERIODIC BROWSER RESTART.
 *
 * Long single sessions crash Chrome's renderer ("Aww Snap") from accumulated memory
 * after hundreds of note round-trips. To stay reliable, the browser is quit and
 * relaunched every RESTART_EVERY downloads. This is safe because the dedup store is
 * saved after every download, so a fresh session simply resumes — already-downloaded
 * records are skipped.
 *
 * Because the driver is replaced mid-run, it is NOT a final field: run() owns the current
 * driver and swaps it. Helpers read it via the `driver` field, which run() updates.
 *
 * Other behavior: worklist-driven per-day loop (visits every pending key; no early exit),
 * date-matched dropdown selection (M/d/yyyy), Exit-dismiss for the finalized-encounter
 * modal, Option-B naming (PatientName_MRN_Date.pdf; dedup on the store), MRN read-back,
 * stale-tolerant reads, honest per-day reconciliation. PRUNE_DAYS (45) > WINDOW_DAYS (30).
 *
 * PHI NOTE — outputs (pdf/xlsx/json) and logs (name+MRN) hold PHI; protect them.
 *
 * Dependencies (Maven): selenium-java, gson, poi-ooxml
 */
public class CheckinextractorbotnewFinalLive {
    static SeleniumUtils sel;
    static Utility utility;
    static String projDirPath;
    static Logger logger = LogManager.getLogger(CheckinextractorbotnewFinalLive.class);
    ...
    private static final Path   AUDIT_FILE   = Paths.get("checkin_audit.xlsx");   // PHI: protect
    private static final Path   DOWNLOAD_DIR = Paths.get("downloaded_notes");     // PHI: protect
    ...
    public CheckinextractorbotnewFinalLive(WebDriver driver) {
        setDriver(driver);
    }
    ...
    public void run() throws IOException, InterruptedException {
        ...
        ExcelTracker audit = new ExcelTracker(AUDIT_FILE);
    ...
    public static void main(String[] args) throws Exception {
        ...
        CheckinextractorbotnewFinalLive bot = null;
        try {
            driver = loginAndOpen();
            bot = new CheckinextractorbotnewFinalLive(driver);
            bot.run();
        } finally {
            // run() may have replaced the driver via restart; quit whatever is current.
